...
symbolicGetContour _ (RectR 0 (P (x1,y1)) (P (x2,y2))) = [map P [ (x1,y1), (x2,y1), (x2,y2), (x1,y2), (x1,y1) ]]
symbolicGetContour res (Circle r) = [[ P ( r*cos(2*pi*m/n), r*sin(2*pi*m/n) ) | m <- [0.. n] ]]
...
symbolicGetContour res (Scale2 s@(a,b) obj) = map (map m2) $ symbolicGetContour (res/sc) obj
	where
		sc = max a b
		m2 (P (x,y)) = P (x*a, y*b)	 
...
symbolicGetContourMesh res (Scale2 (a,b) obj) = map (\(a,b,c) -> (m2 a, m2 b, m2 c))  $
	symbolicGetContourMesh (res/sc) obj
	where
		sc = max a b
		m2 (P (x,y)) = P (x*a, y*b)	 
symbolicGetContourMesh _ (RectR 0 (P (x1,y1)) (P (x2,y2))) = [(P (x1,y1), P (x2,y1), P (x2,y2)), (P (x2,y2), P (x1,y2), P (x1,y1)) ]
